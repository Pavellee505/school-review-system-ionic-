...
function onDisconnectedListener(){
  console.log("QB CHAT => onDisconnectedListener");
}

function onReconnectListener(){
  console.log("QB CHAT => onReconnectListener");
}

function onSystemMessageListener(message) {
  console.log("QB CHAT => onSystemMessageListener");
...
function retrieveChatDialogs() {
  ...
  QB.chat.dialog.list(null, function(err, resDialogs) {
    if (err) {
      ...
      console.log("retrieve dialog list", resDialogs);
      // repackage dialogs data and collect all occupants ids
      //
      if(resDialogs.length == 0) { return; }
...
function showOrUpdateDialogInUI(itemRes, updateHtml) {
  ...
  if (updateHtml === true) {
    ...
    var dialogHtml = '<li><a href="#"  id="' + dialogId + '" onclick="triggerDialog(' + "'" + dialogId + "', '" + opponentName + "'" + ')" qb_id="' + opponentId + '" qb_login="' + opponentLogin + '">';
    
    if(dialogUnreadMessagesCount > 0  && currentDialog._id != dialogId) {
      dialogHtml += '<span id="unread-msg" class="badge badge-warning">' + dialogUnreadMessagesCount + '</span>';
      console.log("Current Dialog ID => " + currentDialog._id + " : Income Dialog ID => " + dialogId);
      total_unread_msg_count += dialogUnreadMessagesCount;
    }

    dialogHtml += '<span class="opponent_user" >' + opponentName + '</span>';
...
// Choose dialog
function triggerDialog(dialogId, opponentName){
  console.log("Select a dialog with id: " + dialogId + ", name: " + dialogs[dialogId].name);

  // Refresh badge number
  $('#'+dialogId+' .badge').text(0).delay(250).fadeOut(500);

  $('#chatDlg .income_box .income_user').html('');
  $('#chatDlg .income_box .messages_list').html('');

  $("#chatDlg .load-msg").show(0);
  // load chat history
  //
  retrieveChatMessages(dialogs[dialogId], null);

  // Refresh total badge number
  total_unread_msg_count -= dialogs[dialogId].unread_messages_count;
  if(total_unread_msg_count < 1) {
    $("#total-unread-msg").text(0).fadeOut(200);
  } else {
    $("#total-unread-msg").text(total_unread_msg_count);
  }

  $(".income_box").css("display", "block");
  $("#chatDlg .modal-title").html('Dialog with ' + opponentName);
...
// create new dialog
function createNewDialog(opponentId, opponentLogin) {
...
function clickToAddMsg(id) {
  console.log("click To add Mag", id);
}
