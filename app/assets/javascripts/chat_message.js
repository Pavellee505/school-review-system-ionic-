...
function onMessage(userId, msg) {
  console.log("QB CHAT => onMessageListener");
...
function onDeliveredStatusListener(messageId) {
  console.log("QB CHAT => onDeliveredStatusListener");

  $('#delivered_'+messageId).fadeIn(200);
}

function onReadStatusListener(messageId) {
  console.log("QB CHAT => onReadStatusListener");
...
function retrieveChatMessages(dialog, beforeDateSent) {
  ...
  QB.chat.message.list(params, function(err, messages) {
    if (messages) {
      ...
      if(messages.items.length === 0) {
        ...
        $(".modal-title").html("Dialog with " + opponentLogin);
        messages.items.forEach(function(item, i, arr) {
          ...
          var messageReadStatus = item.read;

          // send read status
          if (item.read_ids.indexOf(currentUser.id) === -1) {
            console.log("Message Read Status to " + messageSenderLogin);
          ...
          var messageHtml = buildMessageHTML(messageText, messageSenderLogin, messageDateSent, messageAttachmentFileId, messageId, messageReadStatus);

          $('.messages_list').prepend(messageHtml);
          ...
          if (i > 5) {$('.messages_list').scrollTop($('.messages_list').prop('scrollHeight'));}
        });
      }
    }else{
      console.log(err);
  ...
  $("#chatDlg .load-msg").delay(100).fadeOut(500);
}
...
function showUserIsTypingView(isTyping, userId, dialogId) {
  if(isMessageForCurrentDialog(userId, dialogId)){
    ...
    if (!isTyping) {
      ...
      $('.messages_list').append(typingUserHtml);
    }
